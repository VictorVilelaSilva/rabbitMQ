...
Q_MAIN  = "q.main"
Q_DLQ   = "q.dlq"
Q_FAILED = "q.failed"  # fila para mensagens que excederam max retry
...
# Fila para mensagens que excederam o limite de tentativas
ch.queue_declare(Q_FAILED, durable=True)
ch.queue_bind(Q_FAILED, EX_DLX, RK_FAILED)

# DLQ e binding
args = {
  "x-dead-letter-exchange": EX_MAIN,
  "x-dead-letter-routing-key": RK_MAIN,
  "x-message-ttl": 10000,   # opcional: expirar e ir pra DLX depois de 10 segundos
}
ch.queue_declare(Q_DLQ, durable=True, arguments=args)
...
print("Publicado em q.main com retry_count=0")